<%@page import="tool.automator.common.db.models.PageDependencyModel"%>
...
<table id="newspaper-a">
		...
		<tbody>
			...
			for (int i = 0; i < projectDetails.getPageDetailsList().size(); i++) {
				UIPageModel uiPage = projectDetails.getPageDetailsList().get(i).getPage();
			%>
				<tr>
				...
				if(projectDetails.getPageDetailsList().get(i).getPageDependencies().size() > 0) {
					for (int j = 0; j < projectDetails.getPageDetailsList().get(i).getPageDependencies().size(); j++) {
						PageDependencyModel pageDependency = projectDetails.getPageDetailsList().get(i).getPageDependencies().get(j);
				%>
					<% if(j != 0) { %>
						<tr>
						<td></td>
